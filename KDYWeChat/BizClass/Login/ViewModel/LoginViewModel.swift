    ...
    init(input: (username: Driver<String>, password: Driver<String>)) {
        validatedUsername = input.username
            .map { usernameString in
                let usernameRule = ValidationRuleLength(min: 1, max: 50, failureError: ValidationError(message: "Invalid Username"))
                return usernameString.validate(rule: usernameRule)
            }
        
        validatedPassword = input.password
            .map { passwordString in
                let passwordRule = ValidationRuleLength(min: 6, max: 50, failureError: ValidationError(message: "Invalid Password"))
